...
const Products = (props) => {
  ...
  const fetchData = async () => {
    await axios.get("http://localhost:6001/fetch-products").then((response) => {
      if (props.category === "all") {
        setProducts(response.data);
        setVisibleProducts(response.data);
      } else {
        setProducts(
          response.data.filter((product) => product.category === props.category)
        );
        setVisibleProducts(
          response.data.filter((product) => product.category === props.category)
        );
                ...
                {categories.map((category) => {
                  return (
                    <div className="form-check">
                      ...
                      <label
                        className="form-check-label"
                        htmlFor={"productCategory" + category}
                      >
                        {category}
                      </label>
                    </div>
